...
namespace GarmentZone.Screens
{
    public partial class frmAdjustment : Form
    {
        ...
        public void LoadProducts()
        {
            ...
            con.Open();
            cmd = new SqlCommand("select p.pcode, p.barcode, p.pname, b.brand, c.category, v.vendor, p.price, p.qty from tblProduct as p inner join tblBrand as b on b.id = p.bid inner join tblCategory as c on c.id = p.cid inner join tblVendor as v on v.id = p.vendorid  where p.pname like '" + txtSearch.Text + "%'", con);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("STOCK ON HAND QTY SHOULD BE GREATER THAN FROM ADJUSTMENT QTY.", "WARNING", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
